...
interface BonData {
  ...
  date: string;
  bonNum: number;
  signatures: { role: string; signature_url: string | null; signe_le: string }[];
}
...
export default function BonVerificationPage() {
  ...
  useEffect(() => {
    if (!id) return;
    apiGetBon(id)
      .then((data: any) => setBon(data as BonData))
  ...
  if (notFound || !bon) {
    return (
      <div className="min-h-screen bg-gray-100 flex flex-col items-center justify-center px-4 text-center gap-6">
        <img src="/LOGO.webp" alt="RIMATEL" className="w-24 h-24 object-contain border-2 border-green-700 rounded-lg p-2 bg-white" />
  ...
  const getSig = (role: string) => bon.signatures.find((s) => s.role === role);
    ...
    <div className="min-h-screen bg-gray-100 flex flex-col items-center justify-start px-4 py-8">
      <div className="w-full max-w-2xl bg-white rounded-2xl shadow-lg border border-green-200 overflow-hidden">
        ...
        <div className="flex items-center justify-between gap-4 p-5 border-b-2 border-green-700">
          <img src="/LOGO.webp" alt="RIMATEL" className="w-20 h-20 sm:w-28 sm:h-28 object-contain flex-shrink-0 border-2 border-green-700 rounded-lg p-2 bg-white" />
          <div className="flex-1 text-center min-w-0">
            ...
            <p className="font-bold text-gray-800 text-xs sm:text-sm mt-2">Centre d'appel : 28888882</p>
          </div>
                  ...
                  <div className="w-full h-16 flex items-center justify-center border-b border-gray-400 relative">
                    {hasUrl && !hasError ? (
                      <img
                        ...
                        onError={() => setImageErrors(prev => ({ ...prev, [step.role]: true }))}
                      />
                    ) : (
