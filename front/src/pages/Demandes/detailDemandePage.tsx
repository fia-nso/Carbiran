...
export default function DetailDemandePage() {
  ...
  const fetchDemande = useCallback(async (silent = false) => {
    ...
    try {
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      const rawData: any = await apiGetDemande(id);
      ...
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      (rawData.demande_vehicules ?? []).forEach((dv: any) => {
        if (dv.vehicule) {
          vMap[dv.vehicule_id] = {
            id: dv.vehicule.id,
            vehicule: dv.vehicule.vehicule,
            matricule: dv.vehicule.matricule,
            chauffeur_responsable: dv.vehicule.chauffeurResponsable ?? null,
            zone: dv.vehicule.zone,
          };
        }
      });
      setVehiculesMap(vMap);

      // Photos map — incluses dans la réponse API
      const pMap: Record<string, PhotoJustification[]> = {};
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      (rawData.demande_vehicules ?? []).forEach((dv: any) => {
        if (dv.photos && dv.photos.length > 0) {
          pMap[dv.id] = dv.photos as PhotoJustification[];
        }
      });
      setPhotosMap(pMap);
    } catch (e: unknown) {
  ...
  async function handleValiderVehicule(dv: DemandeVehicule) {
    ...
    setSubmitError(null);
    try {
      await apiPatchDemandeVehicule(demande.id, dv.id, { statut: "valide" });

      await apiCreateRavitaillement({
        vehicule_id:        dv.vehicule_id,
      ...
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      const fresh: any = await apiGetDemande(demande.id);
      const tousTraites = (fresh.demande_vehicules ?? []).every(
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        (d: any) => d.statut === "valide" || d.statut === "refuse"
      );
      if (tousTraites) {
        await apiUpdateDemande(demande.id, { statut: "validee_cellule" });
      }
  ...
  async function handleRefuserVehicule(dv: DemandeVehicule) {
    ...
    setSubmitError(null);
    try {
      await apiPatchDemandeVehicule(demande.id, dv.id, { statut: "refuse" });
  ...
  async function handleSoumettreSituation() {
    ...
    setSubmitError(null);
    try {
      await apiUpdateDemande(demande.id, { situation_soumise: true });
      await fetchDemande();
